...
export default function EventsScreen({ navigation }) {
  const [selectedDate, setSelectedDate] = useState(new Date("2026-06-14"));
  const [showDatePicker, setShowDatePicker] = useState(false);
  const formattedDate = selectedDate.toISOString().split("T")[0];
  const events = EVENTS.filter((event) => event.date === formattedDate);
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            {Platform.OS === "web" ? (
              ...
              <DateTimePicker value={selectedDate} mode="date" display="default" onChange={(_, date) => onDateChange(date)} />
            )}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 14, fontWeight: "600", color: "#8C7DA0", marginBottom: 8 },
  datePicker: { flexDirection: "row", alignItems: "center", backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", borderRadius: 12, padding: 12, gap: 8 },
  dateText: { fontSize: 16, color: "#EBE3D6" },
  empty: { alignItems: "center", marginTop: 40 },
  emptyText: { color: "#8C7DA0", fontSize: 14 },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.7)", justifyContent: "center", alignItems: "center" },
  modalContent: { backgroundColor: "#1A1528", borderRadius: 20, padding: 20, width: "90%", alignItems: "center" },
